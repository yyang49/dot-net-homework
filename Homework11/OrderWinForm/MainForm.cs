...
namespace OrderWinForm
{
    public partial class MainForm : Form
    {
        ...
        private void editOrder_Click(object sender, EventArgs e)
        {
            ...
            {
                int index = orderService.GetAll().IndexOf(new Order.Order() { orderNo = (int)dataGridView1.CurrentRow.Cells[0].Value });
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (searchType.Text !="" && filter.Text !="")
            {
                ...
                {
                    case "订单号":
                        searchResult = orderService.SelectOrder(1, filter.Text);
                        break;
                    case "商品名":
                        searchResult = orderService.SelectOrder(2, filter.Text);
                        break;
                    case "收货人":
                        searchResult = orderService.SelectOrder(3, filter.Text);
                        break;
                }
